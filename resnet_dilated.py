...
from paddle.nn import Conv2D, BatchNorm, MaxPool2D, Linear,Layer,Sequential, ReLU, AvgPool2D
    ...
    def __init__(self, layers=50, num_classes=1000):
        ...
        self.layer4 = Sequential(
                *self.make_layer(block,
                                 ...
                                 name='layer4',
                                 dilation=4))
        self.last_pool = AvgPool2D(kernel_size=7 # ignore if global_pooling is True
                                )
...
def ResNet18(pretrained=False):
    ...
    if pretrained:
        model_state, _ = paddle.load(model_path['ResNet18'])
...
def ResNet34(pretrained=False):
    ...
    if pretrained:
        model_state, _ = paddle.load(model_path['ResNet34'])
        model.set_state_dict(model_state) # set_dict(model_state)
    return model
...
def ResNet50(pretrained=False):
    ...
    if pretrained:
        model_state, _ = paddle.load(model_path['ResNet50'])
...
def ResNet101(pretrained=False):
    ...
    if pretrained:
        model_state, _ = paddle.load(model_path['ResNet101'])
        model.set_state_dict(model_state)
    return model
...
def ResNet152(pretrained=False):
    ...
    if pretrained:
        model_state, _ = paddle.load(model_path['ResNet152'])
        model.set_state_dict(model_state)
    return model
